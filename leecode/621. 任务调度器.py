...
class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        ...
        max_let: str = max(t_count, key=lambda x: t_count[x])
        max_count: int = t_count[max_let]
        same_count_: int = 0
        del t_count[max_let]
        # print(t_count)
        for i in t_count:
            if t_count[i] == max_count:
                ...
                same_count_ += 1
        # print(t_count)
        remain_num: int = sum([i for i in t_count.values()])
        return sum(t_count.values()) + max_count if remain_num >= (matrix := n * (max_count - 1)) else matrix + max_count + same_count_
