...
class FileSystem:
    __slots__ = 'disk_block_size', 'each_block_size', 'data_block_size', \
                'file_catalog_size', 'UIF', 'IF', 'program_dict'

    def __init__(self):
        self.disk_block_size: int = 6  # 划分磁盘块数量
        self.each_block_size: int = 512  # 每块磁盘的大小(B)
        self.data_block_size: int = 64  # 每个数据块大小(B)
        self.file_catalog_size: int = 5  # 文件目录大小
    ...
    def launch_file_system(self) -> None:  # 此处是主函数
        current_file_list: list = os.listdir(os.getcwd())

        # print('正在启动文件系统....\n\n启动成功，请选择以下功能:（！！！初次使用请先进行初始化！！！）\n\n')
        if 'file_content' in current_file_list and 'file_system' in current_file_list:
            sign: str = input('系统已保存有初始化文件，是否覆盖？(y/n)')
            if sign in ['N', 'n']:
                self.IF = True
        while (
                op := input('\t0：初始化系统\t1：创建新文件\t2：复制文件\t3：打开文件\t4：删除文件\t5：打开文件目录\t6.检查位示图和文件目录使用情况\n\n请选择:')):
            if op != '0' and not self.IF:
                print('！！！！请先初始化系统！！！！\n\n')
                continue
            if sys_op := self.program_dict.get(int(op)):
                sys_op()  # 根据用户输入从字典中取出可调用对象
            else:
                print('没有找到该操作！！！')
    ...
    def dir() -> None:
        ...
        with open('file_system', 'rb+') as f:
            ...
            print(f'目前共有{disk_block.file_number}个文件:')
    ...
    @staticmethod
    def show_disk():  # 检查位示图和文件目录使用情况
        # 反序列化，从磁盘读取位示图和文件目录
        with open('file_system', 'rb+') as f:
            disk_block: DiskBlock = pickle.load(f)
            file_catalog: List[FileCatalog] = pickle.load(f)
            print('\n\n位示图使用情况：')
            [print(f'| 序号:{i},状态：{bool(j)}', end=' |  ') for i, j in enumerate(disk_block.map)]
            print('\n')
            print('文件目录使用情况：')
            [print(f'| 序号:{i},状态：{not j.valid_sign}', end=' |  ') for i, j in enumerate(file_catalog)]
            print('\n\n')

    def initial_system(self) -> None:
        if self.disk_block_size <= 2:
            print('！！！磁盘划分块数量不足,请至少划分2块以上的磁盘分区！！！')
            self.UIF = True
            return
        ...
        if self.UIF:
            return
        with open('file_content', 'w+') as f:  # 申请文件内容空间
    ...
    def check_space(self, start_index: int, end_index: int) -> int:
        if self.each_block_size <= (end_index - start_index):
            print('！！磁盘空间不足！！无法存入磁盘位示图或文件目录！!')
            print(f'需要的空间大小：{end_index - start_index},你划分的空间大小：{self.each_block_size}!!!请在构造函数中进行修改！！')
            self.UIF = True
        return end_index
    ...
    def request_disk_space_and_write(self,  # 申请磁盘空间和写入
                                     disk_block: DiskBlock,
                                     file_catalog: List[FileCatalog],
                                     head_block: DataBlock,
                                     occupancy_size: int,
                                     text_length: int,
                                     file_catalog_index: int,
                                     file_name: str,
                                     exp_name: str) -> bool:

        # 申请磁盘空间
        w_index: int = 2
        while w_index < self.disk_block_size:
            if not disk_block[w_index]:
                valid_sign: bool = True
                mark: int = w_index
                counter: int = 0
                while w_index < self.disk_block_size and counter < occupancy_size:
                    if disk_block[w_index]:
                        valid_sign = False
                        break
                    w_index += 1
                    counter += 1
                if valid_sign:  # 成功申请到空间
                    ...
                    print(
                        f'找到适配的磁盘空间，第一块序号：{mark},最后一块序号：{mark + occupancy_size - 1}.\n'
                        f'开始将文本内容写入磁盘....')

                    # 设置文件目录
                    file_catalog[file_catalog_index].set_catalog(file_name=file_name, exp_name=exp_name,
                                                                 file_size=text_length, disk_index=mark,
                                                                 occ_num=occupancy_size)

                    # 修改磁盘位示图
                    disk_block.map[mark: mark + occupancy_size] = [1] * occupancy_size
                    # 文件数加一
                    ...
                    with open('file_content', 'r+') as f:
                        f.seek(self.each_block_size * mark, 0)  # 将指针移动到第一块空闲区域位置
    ...
    def make_file(self) -> None:
        ...
        # 申请磁盘空间
        print('文件写入成功！！') if self.request_disk_space_and_write(disk_block=disk_block,
                                                               file_catalog=file_catalog,
                                                               occupancy_size=occupancy_size,
                                                               text_length=text_length,
                                                               file_catalog_index=file_catalog_index,
                                                               file_name=file_name, exp_name=exp_name,
                                                               head_block=head_block) else print('写入失败！！没有足够的磁盘空间！！')
    ...
    def copy(self) -> None:
        ...
        w_index: int = 2
        while w_index < self.disk_block_size:
            print(disk_block[w_index], w_index)
            if not disk_block[w_index]:
                valid_sign: bool = True
                mark: int = w_index
                counter: int = 0
                while w_index < self.disk_block_size and counter < disk_space:
                    if disk_block[w_index]:
                        valid_sign = False
                        break
                    w_index += 1
                ...
                if valid_sign:  # 成功申请到空间
                    ...
                    print(
                        f'找到适配的磁盘空间，第一块序号：{mark},最后一块序号：{mark + disk_space - 1}.\n'
                        f'开始将文本内容写入磁盘....')
                    ...
                    # 修改磁盘位示图
                    disk_block.map[mark: mark + disk_space] = [1] * disk_space
                    # 文件数加一
                    disk_block.file_number += 1
                    ...
                    with open('file_content', 'r+') as f:  # 将原文件内容写入数据块
                        ...
                        # 将数据块中的内容写入磁盘
                        f.seek(mark * self.each_block_size, 0)  # 文件指针移动到申请到空间的第一区域
